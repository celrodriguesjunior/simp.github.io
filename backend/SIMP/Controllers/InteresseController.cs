...
namespace SIMP.Controllers{
    ...
    public class InteresseController : Controller{
        ...
        public async Task<IActionResult> Delete([FromQuery] int id_proposta, [FromQuery] int id_universitario){
            ReturnRequest result = new ReturnRequest();
            try{
                result.Data = await interesseRepository.Desactive(new Interesse(){Nr_id_proposta = id_proposta, Nr_id_universitario = id_universitario});
                if (result.Data == null
                && !((bool) result.Data)){
                    result.Status = "409";
                    return Conflict(result);
        ...
        // PUT interesse
        [HttpPut]
        public async Task<IActionResult> Put([FromBody] Interesse Model){
            ReturnRequest result = new ReturnRequest();
            try{
                result.Data = await interesseRepository.AcceptInterest(Model);
                if (result.Data == null
                || !((bool) result.Data)){
                    result.Status = "409";
                    result.Status = null;
