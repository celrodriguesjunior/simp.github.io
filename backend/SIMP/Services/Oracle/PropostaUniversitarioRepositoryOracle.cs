...
namespace SIMP.Services.Oracle{
    ...
    public class PropostaUniversitarioRepositoryOracle : TableBaseRepositoryOracle, IPropostaUniversitarioRepository{
        ...
        public async Task<PropostaUniversitario> GetByUniversityProposals(int Id_universitario, int Id_proposta){
            return await Connection.QueryFirstOrDefaultAsync<PropostaUniversitario>(
                $@"SELECT * FROM {TBL_PROPOSTA_UNIVERSITARIO.NAME}
                      WHERE {TBL_PROPOSTA_UNIVERSITARIO.NR_ID_UNIVERSITARIO} = {Id_universitario}
                        AND {TBL_PROPOSTA_UNIVERSITARIO.NR_ID_PROPOSTA} = {Id_proposta}");
        }

        public Task<bool> Update(PropostaUniversitario Model){
            throw new NotImplementedException();
        }

        public Task<IEnumerable<PropostaUniversitario>> GetAll(){
            throw new NotImplementedException();
        }
    }
